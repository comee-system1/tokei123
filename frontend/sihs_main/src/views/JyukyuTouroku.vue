    ...
    <v-container fluid class="container">
      <v-row no-gutters class="main-row">
        ...
        <v-col class="center-area ml-1">
          <v-row no-gutters>
            <v-card elevation="0" class="center-area-riyoname d-flex flex-row" flat tile>
          ...
          <v-row no-gutters>
            <v-card elevation="0" class="center-area-jyukyusyuri d-flex flex-row" flat tile>
          ...
          <v-row no-gutters>
            <v-card elevation="0" class="center-area-button-new d-flex flex-row" flat tile>
              <a class="add-button"  style="width: 150px;" @click="openDialog_Term('add_new')"
                >受給者証新規登録</a
              >
              <a
                class="ml-1 copy-button" style="width: 100px;"
                @click="openDialog_Term('copy_last')"
                >前回コピー</a
              >
              <v-card elevation="0" class="d-flex flex-row-reverse" style="width: calc(100% - 250px);" flat tile>
                <label style="font-size: 12px;padding-top: 10px;padding-right: 20px">：必須項目</label>
                <label style="font-size: 12px;color: #ce3e47;padding-top: 8px;font-size: 1.5em;">*</label>
              </v-card>
            </v-card>
          </v-row>
...
export default {
  data() {
    return {
      ...
      tabheight: '30',
      menuitems: [
        { id: 1, name: "基本情報▸", syogai: true, syogaiJi: true, soudan: true },
        { id: 2, name: "障害区分▸", syogai: true, syogaiJi: false, soudan: false },
        { id: 3, name: "支給量▸", syogai: true, syogaiJi: true, soudan: false },
        { id: 4, name: "計画相談▸", syogai: true, syogaiJi: true, soudan: true },
        { id: 5, name: "利用負担▸", syogai: true, syogaiJi: true, soudan: false },
      ],
  ...
  methods: {
    ...
    parent_common_tab_menu: function (args) {
      ...
      // タブを切り替えた際の表示切替制御
      if (args.selectTab == 'JyukyuSyogaiFukusi') this.JyukyuSyogaiFukusiFlag = true;
      if (args.selectTab == 'JyukyuSyogaiJi') this.JyukyuSyogaiJiFlag = true;
      if (args.selectTab == 'JyukyuChiikiSoudan') this.JyukyuChiikiSoudanFlag = true;
