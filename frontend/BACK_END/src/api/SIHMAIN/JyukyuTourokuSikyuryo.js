 ...
 */
 exports.connected = async function (param) {
    // 接続確認用URL
    var url = config.getDomain() + '/syogai/daityo/v1/jyukyu/sikyuketei?getkbn=0&rysid=' + param.rid;
    // var url = config.getDomain() + '/syogai/daityo/v1/jyukyu/sikyuketei?getkbn=0';
    config.setURL(url);
    ...
    return await service.getData().then(result => {
        ...
        result.forEach((element) => {
            skryoh1_inf.push({
                ...
                sksymdDisp: moment(element.sksymd).format('YYYY.MM.DD'),
                skeymd: element.skeymd,         // 支給終了日
                skeymdDisp: element.skeymd===null?'':moment(element.skeymd).format('YYYY.MM.DD'),
                ...
                dspskryo: element.dspskryo,       // 表示用支給量1:基本支給量
                dspskryo2: element.dspskryo2,      // 表示用支給量2:1回当たりの支給量等
            });
