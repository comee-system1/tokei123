    ...
    <v-row no-gutters>
      <v-col>
        <wj-flex-grid
          ...
          :allowSorting="false"
        >
          <div class="servicessyubetu-title">サービス種別</div>
          <wj-flex-grid-column
            :width="150"
            :wordWrap="true"
          ></wj-flex-grid-column>
    ...
    <v-row no-gutters>
      <v-col>
        <wj-flex-grid
          ...
          :allowResizing="false"
          :allowSorting="false"
          ><div class="kyuhu-meisairan-title">給付明細欄</div>
          <wj-flex-grid-column
            :binding="'service'"
            :header="'サービス内容'"
            width="3.5*"
...
export default {
  data() {
    return {
      ...
      // allData: [],
      // griddata: [],
      kyuhuServiceGridData: this.getData(),
  ...
  methods: {
    ...
    onInitializeServicessyubetuGrid: function (flexGrid) {
      ...
      let mm = new wjGrid.MergeManager(flexGrid);
      // 結合するセルの範囲を指定
      let cellRanges = [new wjGrid.CellRange(0, 0, syubetuCount - 1, 0)];
      // getMergedRangeメソッドをオーバーライドする
      mm.getMergedRange = function (panel, r, c) {
        if (panel.cellType == wjGrid.CellType.Cell) {
          for (let h = 0; h < cellRanges.length; h++) {
    ...
    onInitialized: function (grid) {
      ...
      let headerRowRanges = [
        new wjGrid.CellRange(0, 0, griddata.length - 1, 0),
      ];
