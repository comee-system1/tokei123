...
export default {
  ...
  methods: {
    ...
    setEvents(flexGrid) {
      let _self = this;
      let _hojomode = this.hojomode;
      //セルをクリック
      flexGrid.hostElement.addEventListener('click', function (e) {
        let hPage = flexGrid.hitTest(e.pageX, e.pageY);
        _self.$emit('child_data', _self.list[hPage.row], _hojomode);
      });
    },
    ...
    createHeader(flexGrid) {
      ...
      // ヘッダ記載
      for (let i = 0; i < 2; i++) {
        flexGrid.columnHeaders.setCellData(0, i, this.title);
      }
